...
export class SurgeryService {
  constructor(
    @InjectRepository(SurgeryQueueItem) private readonly queue: Repository<SurgeryQueueItem>,
  ...
  async findAll(dto: QuerySurgeryDto): Promise<PageResult<unknown>> {
    ...
    qb.orderBy('s.surgeryDate', dto.sortDir === 'DESC' ? 'DESC' : 'ASC', 'NULLS LAST')
      .addOrderBy('s.id', 'ASC');
    const [items, total] = await qb.getManyAndCount();
    return PageResult.of(items.map((i) => this.toResponse(i)), total, dto);
  }
  ...
  async create(dto: UpsertSurgeryDto, userId: string | null): Promise<unknown> {
    const item = new SurgeryQueueItem();
    await this.assign(item, dto);
    const saved = await this.queue.save(item);
    await this.audit.record({
      userId, action: 'create', entity: 'surgery_queue', entityId: saved.id,
      changes: { recordedName: saved.recordedName, registryNo: saved.implantRegistryNo },
    });
    return this.findOne(saved.id);
  }

  async update(id: string, dto: Partial<UpsertSurgeryDto>, userId: string | null): Promise<unknown> {
    const item = await this.queue.findOne({ where: { id } });
    if (!item) throw AppException.notFound(ErrorCode.SurgeryItemNotFound);
    await this.assign(item, dto);
    await this.queue.save(item);
    await this.audit.record({
      userId, action: 'update', entity: 'surgery_queue', entityId: id,
  ...
  async remove(id: string, userId: string | null): Promise<void> {
    ...
    await this.audit.record({
      userId, action: 'delete', entity: 'surgery_queue', entityId: id,
   ...
   */
  private async assign(item: SurgeryQueueItem, dto: Partial<UpsertSurgeryDto>): Promise<void> {
    if (dto.recordedName !== undefined) item.recordedName = dto.recordedName ?? '';
    ...
    if (dto.prosthesisDue !== undefined) item.prosthesisDue = dto.prosthesisDue ?? null;
    if (dto.status !== undefined) item.status = dto.status;
    if (dto.notes !== undefined) item.notes = dto.notes ?? null;
    if (dto.implantRegistryNo !== undefined) item.implantRegistryNo = dto.implantRegistryNo ?? null;
    ...
    if (dto.implantCaseId !== undefined) {
      ...
      implantCase = dto.implantCaseId
        ? await this.implants.findOne({ where: { id: dto.implantCaseId } })
        : null;
    } else if (dto.implantRegistryNo) {
      implantCase = await this.implants.findOne({
        where: { registryNo: dto.implantRegistryNo },
      });
      item.implantCaseId = implantCase?.id ?? null;
    } else if (item.implantCaseId) {
      implantCase = await this.implants.findOne({ where: { id: item.implantCaseId } });
    }
    ...
    if (implantCase && item.recordedName) {
      ...
      if (!item.implantRegistryNo) item.implantRegistryNo = implantCase.registryNo;
    } else {
      item.hasNameMismatch = false;
    }

    item.searchText = searchKey(
      [item.implantRegistryNo, item.recordedName, item.toothPosition, item.implantBrand]
        .filter(Boolean)
        .join(' '),
    );
  }

  private formatDate(value: Date | string, precision: DatePrecisionEnum | null): string {
    return (
    ...
    return {
      ...
      patient: item.implantCase?.patient
        ? {
            ...
            fullName: `${item.implantCase.patient.firstName} ${item.implantCase.patient.lastName}`.trim(),
            mobile: item.implantCase.patient.mobile,
          }
        : null,
      surgeryDate: item.surgeryDate
        ? {
            jalali: this.formatDate(item.surgeryDate, item.surgeryDatePrecision),
            iso: JalaliDate.fromDate(new Date(item.surgeryDate))?.toIsoDate() ?? '',

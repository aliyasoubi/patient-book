...
import { SheetRow, WorkbookPort } from '../../../application/ports/workbook.port';
...
export class ExcelJsWorkbookReader extends WorkbookPort {
  ...
  rows(sheetName: string): SheetRow[] {
    ...
    sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      ...
      rows.push({
        rowNumber,
        cell: (column: number) => ExcelJsWorkbookReader.text(row.getCell(column).value),
      });
    });
    return rows;
  }
  ...
  private static text(value: ExcelJS.CellValue): string {
    ...
    if (value instanceof Date) return JalaliDate.fromDate(value)?.toIsoDate() ?? '';

    if (typeof value === 'object') {
      if ('text' in value && typeof value.text === 'string') return value.text.trim();
      if ('result' in value) return ExcelJsWorkbookReader.text(value.result as ExcelJS.CellValue);
